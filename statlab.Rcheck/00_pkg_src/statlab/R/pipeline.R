...
st_pipeline <- function(config_path) {
  ...
  # tar_progress() distingue les cibles reellement recalculees ("completed")
  # de celles court-circuitees ("skipped") parce que leurs dependances,
  # bien que candidates a l'obsolescence, se sont revelees inchangees une
  # fois reevaluees : c'est ce mecanisme (interne a {targets}) qui assure
  # qu'un changement dans une seule section de config.yml ne recalcule pas
  # les etapes qui n'en dependent pas reellement.
  progres <- tryCatch(targets::tar_progress(), error = function(e) NULL)
  meta <- tryCatch(targets::tar_meta(), error = function(e) NULL)
  execute <- if (!is.null(progres) && !is.null(meta)) {
    noms_executes <- progres$name[progres$progress %in% c("completed", "built")]
    meta[meta$name %in% noms_executes, c("name", "seconds"), drop = FALSE]
  } else {
...
#' rien : lecture seule.
#'
#' Le statut "a recalculer" est une estimation prudente (celle de
#' [targets::tar_outdated()]) : une cible peut y figurer parce qu'un de ses
#' ancetres a change, meme si son propre resultat se revele au final
#' identique une fois recalcule (auquel cas [st_pipeline()] la sautera
#' quand meme a l'execution reelle). Pour le detail exact de ce qui a ete
#' reellement reexecute, se referer a la valeur de retour de
#' [st_pipeline()].
#'
...
st_status <- function(config_path) {
  ...
  if (nrow(a_recalculer) > 0) {
    ...
    cli::cli_alert_info("Estimation prudente : une etape listee ici sera sautee a l'execution si son resultat s'avere finalement inchange.")
