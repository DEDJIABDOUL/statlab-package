...
list(
  ...
  # script et attestation sont des cibles independantes l'une de l'autre
  # (aucune arete entre elles) : {targets} peut les executer dans un ordre
  # quelconque. st_export_script() cree son repertoire de sortie si besoin,
  # mais pas st_attestation() : chaque cible cree donc elle-meme son
  # repertoire, plutot que de dependre de l'ordre d'execution de l'autre.
  targets::tar_target(script, {
    donnees_finales
    dir.create(file.path(.project_dir, "sorties", "rapport"), recursive = TRUE, showWarnings = FALSE)
